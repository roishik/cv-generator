...
import { LayoutDashboard, Wand2, FileText, User, Settings, ShieldCheck } from "lucide-react";
...
export function NavRail({ showAdmin = false }: { showAdmin?: boolean }) {
  ...
  return (
    <nav
      ...
      className={cn(
        "border-border bg-card flex h-full w-[240px] shrink-0 flex-col border-r py-4",
          ...
          return (
            <li key={item.href}>
              ...
              >
                ...
                {isActive && (
                  <span
                    className="bg-spruce-600 ml-auto h-4 w-0.5 rounded-full dark:bg-[hsl(var(--primary))]"
                    aria-hidden
                  />
                )}
              </Link>
